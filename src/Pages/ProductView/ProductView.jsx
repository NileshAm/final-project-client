...
function ProductView() {
  ...
  const [quantity,setQuantity] = useState(1);
   ...
   axios.post(getServerURL('/cart/add'),formData)
    .then(response => {
      ...
      <Container className='mt-3 bg-light'>
        ...
        <Row>
          ...
          <h2 className='text-success mt-3'>
            { numberWithCommas((data.Price)-(data.Price*data.Discount/100))}
          </h2>
          </Col>
          ...
          {/* To-do */}
          <h3 className='text-danger '>{numberWithCommas(data.Price*data.Discount/100)}</h3>
         
          
          <Row className='mt-3 d-grid gap-2 mx-auto mb-2 '>
           
           {/* Make into loding state button */}
            <button className='btn btn-outline-success' onClick={()=>submit('buy')}>Buy</button>
            <button className='btn btn-outline-success' onClick={()=>submit('cart')}>Add to cart</button>
          </Row>
